...
--
-- Purpose:
-- Create a governed voluntary ownership-transfer request.
--
-- Architectural rules:
-- - This command does not change current ownership.
-- - Only the current active canonical owner may initiate a request.
-- - The proposed owner must be an active member of the same company.
-- - Only one pending request may exist per company.
-- - Expired pending requests are closed before a new request is created.
-- - Request creation and audit evidence are written atomically.
-- ============================================================================
...
begin
  ...
  if target_user_id is null then
    return jsonb_build_object(
      ...
      'error_message', 'A proposed owner is required.'
    );
  end if;
  ...
  if target_user_id = actor_user_id then
    return jsonb_build_object(
      ...
      'error_message', 'Ownership cannot be transferred to the current owner.'
    );
  end if;

  normalized_next_role :=
    lower(trim(coalesce(previous_owner_next_role, '')));

  if normalized_next_role not in (
    'admin',
    'member',
    'viewer'
  ) then
    return jsonb_build_object(
      ...
      'error_message',
        'The previous owner role must be admin, member, or viewer.'
    );
  end if;

  if expires_in_hours is null
     or expires_in_hours < 1
     or expires_in_hours > 168 then
    return jsonb_build_object(
      'success', false,
      'error_code', 'INVALID_EXPIRATION',
      'error_message',
        'Transfer expiration must be between 1 and 168 hours.'
    );
  end if;
  ...
  if normalized_reason is not null
     and length(normalized_reason) > 2000 then
    return jsonb_build_object(
      'success', false,
      'error_code', 'INVALID_TRANSFER_REASON',
      'error_message',
        'Transfer reason must not exceed 2000 characters.'
    );
  end if;
  ...
  /*
   * Resolve the actor through the current legacy workspace attachment,
   * then require an active owner membership for the same company.
   *
   * profiles.company_id remains a temporary migration projection.
   */
  select *
  into actor_profile
  ...
  if not found or actor_profile.company_id is null then
    return jsonb_build_object(
      'success', false,
      'error_code', 'OWNER_MEMBERSHIP_REQUIRED',
      'error_message',
        'An active owner workspace membership is required.'
    );
  end if;
  ...
  where id = actor_membership.company_id
  for update;
  ...
  if not found then
    return jsonb_build_object(
      ...
      'error_message', 'The company could not be found.'
    );
  end if;

  /*
   * During migration, companies.user_id must agree with the active owner
   * membership before a voluntary transfer may be initiated.
   */
  if company_record.user_id is distinct from actor_user_id then
    return jsonb_build_object(
      ...
      'error_message',
        'Canonical ownership and legacy ownership projections are inconsistent.'
    );
  end if;
  ...
  if target_membership.workspace_role = 'owner' then
    return jsonb_build_object(
      ...
      'error_message',
        'The proposed owner already holds the owner role.'
    );
  end if;

  /*
   * Close an expired pending request before checking whether the company
   * already has an unresolved transfer.
   */
  ...
  if exists (
    ...
    return jsonb_build_object(
      ...
      'error_message',
        'This company already has a pending ownership-transfer request.'
    );
  end if;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'previous_owner_next_role', normalized_next_role,
...
exception
  ...
  when others then
    raise log
      'request_company_ownership_transfer failed for actor %, target %: %',
      actor_user_id,
      target_user_id,
      sqlerrm;
...
revoke all
on function public.request_company_ownership_transfer(
  uuid,
  text,
  text,
  integer
