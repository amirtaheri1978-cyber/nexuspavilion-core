...
-- Cursor 05G / R-43: deliver Addendum Activity to established respondent
-- companies (S1 quotes + S2 rfq_rfis + S3 prior acknowledgements).
-- Extends addendum_published fanout only. Preserves issuer confirmation,
-- early ADDENDUM_PUBLISHED idempotency, SECURITY DEFINER + search_path='',
-- auth.uid() actor derivation, and all other activity kinds. Does NOT
-- restore authenticated INSERT on notifications / audit_logs. Does NOT
-- use invitation email→profile bridges or open-market broadcast.
