...
export function buildRfiResponseEmail({
  ...
  const displayTitle = safeValue(rfqTitle, "Procurement RFQ");
  const safeTitle = escapeHtml(displayTitle);
  const safeWorkspaceUrl = escapeHtml(workspaceUrl);

  const subject = `Private RFI response available — ${displayTitle}`;

  const text = `Private RFI Response

The issuing procurement team has responded to your private RFI for ${displayTitle}.

Open the secure RFQ workspace to review the response inside Nexus Pavilion:
${workspaceUrl}

Confidentiality notice:
Private RFI correspondence remains confidential between your organization and the issuing procurement team. Competing suppliers cannot view this inquiry or its response.
...
<title>Private RFI Response</title>
</head>
...
<p style="margin:14px 0 0;color:#cbd5e1;font-size:14px;line-height:1.8;font-weight:600;">
Open the RFQ workspace in Nexus Pavilion to review the private RFI response. Response details are not included in this notification.
</p>
</td>
</tr>
</table>
...
<p style="margin:14px 0 0;color:#cbd5e1;font-size:14px;line-height:1.8;font-weight:600;">
This private RFI response notification is part of the Nexus Pavilion procurement record and does not disclose inquiry or response content by email.
</p>
</td>
</tr>
</table>
